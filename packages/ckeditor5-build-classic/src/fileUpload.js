...
export default class FileUpload extends Plugin {
	...
	init() {
		...
		editor.ui.componentFactory.add("fileUpload", (locale) => {
			...
			const regExpSafeNames = fileTypes.map((type) =>
				type.replace("+", "\\+").replace("/", "/")
			);
			const fileTypesRegExp = new RegExp(
				`^(${regExpSafeNames.join("|")})$`
			);
			...
			view.on("done", (evt, files) => {
				//filter allowed file type
				const filesToUpload = Array.from(files).filter((file) =>
					fileTypesRegExp.test(this.getExt(file.name))
				);
				if (filesToUpload.length) {
					//check if file is video type
					let fileExtension = filesToUpload[0].name
						.substr(filesToUpload[0].name.lastIndexOf("."))
						.toLowerCase();
					if (
						fileExtension == ".mp4" ||
						fileExtension == ".ogv" ||
					...
					const fileRepository = editor.plugins.get(FileRepository);
					model.change((writer) => {
						const filesToProcess = Array.isArray(filesToUpload)
							? filesToUpload
							: [filesToUpload];
						for (const fileToProcess of filesToProcess) {
							...
							const placeholderElement = writer.createElement(
								"paragraph",
								{ uploadId: loader.id }
							);
							writer.insertText(
								editor.config.get(
									"file.upload.placeholderText"
								),
								placeholderElement
							);
					...
					throw new CKEditorError(
						"file-upload: File extension is not allowed. Allowed file types = [" +
		...
		doc.on("change", () => {
			...
			for (const entry of changes) {
				if (entry.type == "insert" && entry.name != "$text") {
					...
					for (const filteredItem of filterFromChangeItem(
						editor,
						item
					)) {
		...
		return loader
			...
			.then((data) => {
				model.enqueueChange("transparent", (writer) => {
					...
					const linkedText = writer.createText(
					...
					if (this.isVideo) {
						...
						model.change((writer) => {
							const elem = writer.createElement("video", {
								src: linkFile,
								controls: true,
								width: "100%",
							});
...
function filterFromChangeItem(editor, item) {
	return Array.from(editor.model.createRangeOn(item))
		.filter((value) => {
			return (
				value.item.hasAttribute("uploadId") ||
				value.item.is("element", "paragraph")
			);
		})
